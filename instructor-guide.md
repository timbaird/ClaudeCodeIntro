...
Run through the full OpenRouter setup on a clean machine (or at least verify the environment variables and model work). Free models on OpenRouter change frequently — confirm that `qwen/qwen3-coder:free` is still available and free before class.
...
| Model gives garbage responses | Free model quality varies. Try `/clear` and rephrase. If persistent, try switching models: `/model qwen/qwen3-coder:free` |
